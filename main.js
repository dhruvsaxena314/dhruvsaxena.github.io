// main.js  (v2 — enlarged photo, green accents, LinkedIn + animated stats)
const ACC_DARK = '#334443';
const ACC = '#016B61';

function getBlogs() { return JSON.parse(localStorage.getItem("blogs") || "[]"); }
function saveBlogs(blogs) { localStorage.setItem("blogs", JSON.stringify(blogs)); }
...
const PROFILE = {
  ...
  miniSummary:
    "Young AI researcher from Varanasi merging scientific research, open-source ML, and leadership to build ethical, reproducible AI tools.",
  summary:
    "ResearchGate Fellow with background in AI, NLP, CV and academic collaboration (IIT-BHU, Cambridge). Passionate about reproducible and human-centric AI.",
  accomplishments: [
    "ResearchGate Fellow",
    "ImagoPedia (AI Tool)",
    "Next Voters Fellow",
  ...
  education: {
    ...
    notes:
      "IMO & IEO Gold, Scholarship holder, Founding member of Young Society Science Club. Focused on Programming, AI & ML, Physics, Mathematics."
  },
  projects: [
...
function App() {
  ...
  React.useEffect(() => {
    ...
    let idx=0;
    function tick(){
      if(idx>=stages.length){setLoadedPct(100);setTimeout(()=>setOpened(true),400);return;}
      const s=stages[idx];const start=performance.now();
      function step(now){
        const t=Math.min(1,(now-start)/s.dur);const eased=1-Math.pow(1-t,3);
        const add=Math.round(eased*s.rate*100);
        setLoadedPct(p=>Math.min(99,p+add));
  ...
  // Animate counters digit by digit
  React.useEffect(() => {
    if(!opened)return;
    const target={papers:8,projects:6,awards:9};
    const start=performance.now();
    function frame(now){
      const t=Math.min(1,(now-start)/1800);
      const ease=t<0.5?2*t*t:-1+(4-2*t)*t;
      setStats({
        papers:Math.round(target.papers*ease),
        projects:Math.round(target.projects*ease),
        awards:Math.round(target.awards*ease)
      });
  ...
  function addBlog(title,content){
    const b={id:Date.now(),title,content,date:new Date().toLocaleString()};
    ...
    return (
      <div className="max-w-5xl mx-auto p-6 space-y-6">
        <div className="flex items-center justify-between">
          ...
          <h1 className="text-3xl font-bold text-[var(--accent)]">Blogs</h1>
        </div>
  ...
  return (
    <div className="relative max-w-6xl mx-auto px-6 py-10 space-y-6" style={{'--accent':ACC,'--accent-dark':ACC_DARK}}>
      ...
      {showMenu && (
        <div className="fixed top-14 left-4 bg-[var(--accent-dark)]/80 backdrop-blur-md rounded-xl p-3 space-y-2 z-40">
          ...
          {!authorized && <button className="block w-full text-left hover:bg-[var(--accent)]/20 rounded px-2 py-1" onClick={()=>{
            const email=prompt("Enter authorized email:"); 
            if(email==="dhruvisgood13@gmail.com"){setAuthorized(true);alert("Blog editor unlocked.");} else alert("Access denied.");
          }}>🔒 Admin Login</button>}
      ...
      <header className="flex flex-col md:flex-row justify-between items-center gap-3">
        <div className="flex items-center gap-6">
          <div className="relative">
            <img src="profile.jpg" alt="Dhruv Saxena" className="w-44 h-44 rounded-3xl border-4 border-[var(--accent)]/80 object-cover shadow-xl"/>
      ...
      <section className="grid md:grid-cols-3 gap-6">
        <aside className="space-y-4">
          <div className="bg-white/10 p-4 rounded-xl">
            <h2 className="font-semibold mb-2 text-[var(--accent)]">Profile</h2>
        ...
        <main className="md:col-span-2 space-y-4">
          ...
          <Card title="Education">
            ...
            <p className="text-xs text-white/70 mt-1">{PROFILE.education.notes}</p>
          </Card>
...
function Card({title,children}) {
  return (
    <div className="bg-white/10 p-4 rounded-xl">
      <h2 className="font-semibold mb-2 text-[var(--accent)]">{title}</h2>
      {children}
    </div>
  );
}
function Stat({label,value}) {
  return (
    <div>
      <div className="text-4xl font-bold tracking-wide">{value}</div>
...
function SkillsBlock() {
  return (
    <div className="bg-white/10 p-4 rounded-xl">
      <h2 className="font-semibold mb-3 text-[var(--accent)]">Skills</h2>
      {PROFILE.skills.map(g=>(
        <div key={g.group} className="mb-3">
          <div className="text-sm font-medium mb-2">{g.group}</div>
          {g.items.map(it=>(
            <div key={it.name}>
              <div className="flex justify-between text-xs mb-1">
                <div>{it.name}</div><div className="text-white/60">{it.pct}%</div>
              </div>
              <div className="w-full h-2 rounded-full bg-[var(--accent)]/20 overflow-hidden">
                <div className="h-full bg-[var(--accent)] rounded-full" style={{width:`${it.pct}%`}}></div>
              </div>
            </div>
          ))}
        </div>
...
function LangBlock() {
  return (
    <div className="bg-white/10 p-4 rounded-xl">
      <h2 className="font-semibold mb-3 text-[var(--accent)]">Languages</h2>
      {PROFILE.languages.map(l=>(
        <div key={l.name}>
          <div className="flex justify-between text-xs mb-1"><div>{l.name} ({l.prof})</div><div>{l.pct}%</div></div>
          <div className="w-full h-2 rounded-full bg-[var(--accent)]/20">
            <div className="h-full bg-[var(--accent)] rounded-full" style={{width:`${l.pct}%`}}></div>
          </div>
        </div>
      ))}
    </div>
...
function AddBlogForm({onAdd}){
  ...
  return(
    <div className="bg-white/10 p-4 rounded-xl mb-4">
      <h3 className="font-semibold mb-2 text-[var(--accent)]">➕ Add Blog</h3>
      <input value={title} onChange={e=>setTitle(e.target.value)} placeholder="Title" className="w-full p-2 mb-2 bg-white/10 rounded"/>
      <textarea value={content} onChange={e=>setContent(e.target.value)} rows="6" placeholder="Content" className="w-full p-2 bg-white/10 rounded mb-2"></textarea>
      <button onClick={()=>{if(title&&content){onAdd(title,content);setTitle("");setContent("");}}} className="px-3 py-2 bg-[var(--accent)] rounded-md text-white">Save</button>
    </div>
  );
}
